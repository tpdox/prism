...
```bash
codex exec -a "read" -p "Review the following code for bugs, security issues, and quality. Be specific about line numbers and provide severity ratings (critical/high/medium/low).
...
```bash
codex exec -a "read" -p "Review this diff for bugs, logic errors, and improvements:
...
```bash
codex exec -a "read" -p "Review this pull request diff. Focus on correctness, security, and maintainability:
...
- Always use read-only mode (`-a "read"`) — review agents should never modify code.
